...
const mbtilesDir = config.get('mbtilesDir_sat') //edited 2020-01-22
const logDir = config.get('logDir')
const propertyBlacklist = config.get('propertyBlacklist')
let conversionTilelist = config.get('saturdayTilelist') //edited 2021-01-22
...
winston.configure({
  ...
  transports: [ 
    new DailyRotateFile({
      filename: `${logDir}/produce-saturday-%DATE%.log`,
...
const main = async () => {
  winston.info(`${iso()}: Saturday tile production started.`)
  await getScores()
  queueTasks()
  queue.on('drain', () => {
    shutdown()
