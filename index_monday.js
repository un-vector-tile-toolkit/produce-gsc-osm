...
const mbtilesDir = config.get('mbtilesDir_mon') //edited 2020-01-22
const logDir = config.get('logDir')
const propertyBlacklist = config.get('propertyBlacklist')
let conversionTilelist = config.get('mondayTilelist') //edited 2021-01-22
...
winston.configure({
  ...
  transports: [ 
    new DailyRotateFile({
      filename: `${logDir}/produce-monday-%DATE%.log`,
...
const main = async () => {
  winston.info(`${iso()}: Monday tile production started.`)
  await getScores()
  queueTasks()
  queue.on('drain', () => {
    shutdown()
