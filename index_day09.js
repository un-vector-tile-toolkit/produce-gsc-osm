...
const mbtilesDir = config.get('mbtilesDir_day09') //edited 2020-02-12
const logDir = config.get('logDir')
const propertyBlacklist = config.get('propertyBlacklist')
let conversionTilelist = config.get('day09Tilelist') //edited 2021-02-12
...
winston.configure({
  ...
  transports: [ 
    new DailyRotateFile({
      filename: `${logDir}/produce-day09-%DATE%.log`,
...
const main = async () => {
  winston.info(`${iso()}: day09 tile production started.`)
  await getScores()
  queueTasks()
  queue.on('drain', () => {
    shutdown()
