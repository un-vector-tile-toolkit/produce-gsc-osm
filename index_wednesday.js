...
let conversionTilelist = config.get('wednesdayTilelist') //edited 2021-01-22
...
winston.configure({
  ...
  transports: [ 
    new DailyRotateFile({
      filename: `${logDir}/produce-wednesday-%DATE%.log`,
...
const main = async () => {
  winston.info(`${iso()}: Wednesday tile production started.`)
  await getScores()
  queueTasks()
  queue.on('drain', () => {
    shutdown()
